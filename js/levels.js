...
const LEVELS = [
  ...
  },
  {
    ...
    polygons: [
      [
        // ceiling, left to right
        [-5, -8], [29.9, -8],
        [31.7, 4.55],  // the skewer point, hanging over the jump arc
        [33.5, -8], [110, -8],
        // right wall
        [110, 6.4],
        // floor, right to left
        ...
        [32.5, 11.6],                     // gorge floor under the platform
    ...
    burgers: [
      ...
      [36.5, 6.65],  // on the gorge platform
  ...
  {
    name: 'Sriracha Spiral',
    theme: 'volcano',
    // The floor tips away past the warmup roller and dives 43 degrees
    // into a spiral corridor bored into the volcano's flank: two and a
    // quarter turns curling in around the core, ridden flat out on the
    // outer wall through two full inversions. The slot SNUGS the deeper you
    // go -- a forgiving ~0.8 head margin at the wide first loop tightening
    // to a near-lethal ~0.45 at the core, where the least drift off the
    // ideal line buries your head in the rock arm (the arm itself thickens,
    // ~1.8->0.8, as the windings wrap in). So speed alone won't carry you:
    // the dive banks more than enough to hold the first, widest inversion,
    // every lap inward needs less, but the tightening tube punishes a
    // sloppy flat-out line. The corridor dead-ends at the heart of the
    // spiral, where the bucket waits just before the cap. Slot walls
    // generated by _scratch_gen_1_9.js.
    polygons: [
      [
        // ceiling: open sky
        [-5, -8], [62, -8],
        // right wall
        [62, 1],
        // massif dome, right to left
        [57, 1.2], [52.5, 0.7], [48.5, 0.5], [45, 0.7], [42.2, 1.6], [40.5, 2.5],
        // left shoulder down to the overhang beak
        [38.5, 3.5], [36, 4.6], [33.6, 6.2], [31.7, 8], [30.3, 10],
        // flank: the dive corridor's inner wall, down to the arm tip
        [32.89, 12.47], [34.5, 13.97], [36.11, 15.46], [37.73, 16.96],
        // inner trace: corridor ceiling spiraling in (arm faces). Pulled in
        // ~0.33 vs the first cut so the tube is a precision slot -- the bot
        // threads it with ~0.45 head margin; any drift clips the arm.
        [39.47, 18.58], [40.01, 19.03], [40.58, 19.43], [41.18, 19.79], [41.81, 20.09], [42.47, 20.35],
        [43.14, 20.55], [43.82, 20.69], [44.52, 20.78], [45.22, 20.82], [45.92, 20.79], [46.61, 20.7],
        [47.3, 20.56], [47.97, 20.36], [48.62, 20.11], [49.24, 19.8], [49.84, 19.43], [50.41, 19.02],
        [50.93, 18.55], [51.41, 18.04], [51.84, 17.49], [52.22, 16.91], [52.55, 16.29], [52.81, 15.64],
        [53.02, 14.97], [53.16, 14.29], [53.23, 13.59], [53.23, 12.89], [53.17, 12.19], [53.04, 11.51],
        [52.84, 10.84], [52.57, 10.19], [52.24, 9.57], [51.85, 8.99], [51.4, 8.46], [50.9, 7.97],
        [50.35, 7.54], [49.75, 7.17], [49.12, 6.86], [48.46, 6.63], [47.78, 6.47], [47.09, 6.38],
        [46.39, 6.37], [45.7, 6.43], [45.01, 6.58], [44.35, 6.79], [43.71, 7.09], [43.12, 7.45],
        [42.57, 7.88], [42.08, 8.38], [41.65, 8.93], [41.29, 9.53], [41.02, 10.17], [40.82, 10.85],
        [40.72, 11.54], [40.71, 12.24], [40.8, 12.93], [40.97, 13.6], [41.25, 14.25], [41.6, 14.85],
        [42.04, 15.39], [42.56, 15.87], [43.13, 16.26], [43.76, 16.56], [44.43, 16.77], [45.12, 16.88],
        [51.97, 20.91], [51.41, 21.34], [50.83, 21.73], [50.21, 22.08], [49.58, 22.38], [48.93, 22.65],
        [48.26, 22.86], [47.58, 23.04], [46.89, 23.17], [46.19, 23.26], [45.49, 23.3], [44.79, 23.29],
        [44.08, 23.24], [43.39, 23.15], [42.7, 23.01], [42.02, 22.83], [41.35, 22.6], [40.7, 22.34],
        [40.07, 22.03], [39.45, 21.68], [38.86, 21.3], [38.3, 20.88], [37.76, 20.43], [37.62, 20.3],
        // the dive, right to left up to the rollover
        [36.01, 18.81], [34.4, 17.31], [32.79, 15.81], [31.18, 14.32], [29.91, 13.34], [28.55, 12.49],
        [27.12, 11.77], [25.63, 11.19], [23.37, 10.37], [21.12, 9.55], [18.86, 8.73], [16.61, 7.91],
        [14.35, 7.09],
        // warmup roller, start shelf
        [11, 6.7], [8, 8], [0, 8],
        // left wall
        [-5, 8],
      ],
    ...
    start: { x: 2.5, y: 7.25 },
    burgers: [
      [12.5, 6.2],   // warmup roller crest
      [47.2, 22.1],  // bottom of the first lap, the spiral's deepest point
      [55.1, 12.6],  // first lap's right wall, mid-climb
      [46.3, 4.6],   // first inversion: collected riding upside down
      [38.95, 12.6], // second lap's left wall, on the way back down
      [47, 18.3],    // bottom of the second lap
      [51.2, 12.6],  // second lap's right wall
      [46.3, 8.1],   // second inversion, one winding deeper
      [42.35, 12.6], // final lap's left wall, hugging the core
      // the goal sits at the heart, just before the dead-end cap
    ],
    goal: [45.6, 15.35],
  },
  {
    ...
    // floating slab — a fang hangs at head
    // height over the landing, so the arc must be kept low (duck it) or it
    // spikes you; undershooters deflect into the gorge,
    ...
    polygons: [
      [
        ...
        [74.5, -12], [88.2, -12],
        [89.6, -3.2],                       // the sky fang: hangs at head
        [91, -12],                          // height over the slab -- duck it
        [93.5, -20], [231, -20],
        // right wall
        [231, 7.8],
        // floor, right to left
        ...
        [186.5, 13.7], [185.4, 13.7],       // crown 3, now split by THE REAPER'S
        [185.4, 20], [184, 20],             // CLEFT: a glass-walled crevasse. The
        [184, 13.7], [183, 13.7],           // wall's glass runs right up to its
                                            // right lip, so a rider who stalls on
                                            // the climb slides back down the glass
                                            // and drops in -- but at speed the bot
                                            // skips the 1.4m gap clean
    ...
    // obsidian floor edges ([poly, edge] pairs): the dive and pull-out, the
    // two waves, and the wall — the bare-basalt crowns between them (the index
    // gaps) keep their grip. Generated from the old x-spans by
    // _scratch_glass_convert.js, so the classification is byte-identical.
    glassEdges: [
      [0, 18], [0, 19], [0, 20], [0, 21], [0, 22], [0, 23], [0, 24], [0, 25],
      [0, 26], [0, 27], [0, 28],                                   // glass dive + pull-out
      ...
      [0, 64], [0, 65], [0, 66], [0, 67], [0, 68], [0, 69], [0, 70],   // wave 1
      ...
      [0, 114], [0, 115], [0, 116], [0, 117], [0, 118], [0, 119], [0, 120], // wave 2 + crevasse
      [0, 134], [0, 135], [0, 136], [0, 137], [0, 138], [0, 139], [0, 140],
      [0, 141], [0, 142], [0, 143], [0, 144], [0, 145], [0, 146],      // the glass wall
    ],
    start: { x: 2.5, y: 7.25 },
    burgers: [
      ...
      [165, 9.4],     // wave 2's upslope (moved off the crevasse: a burger
                      // on the descent would brake the bot out of the skip)
